...
serverConfigurations.forEach((config) => {
  maxWeight = Math.max(maxWeight, config.weight);
});
for (let i = 0; i < serverConfigurations.length; i++) {
  if (gcd === 0) {
    gcd = serverConfigurations[i].weight;
  } else {
    gcd = gcd_func(gcd, serverConfigurations[i].weight);
  }
}
...
function nextServer() {
  while (true) {
    currentIndex = (currentIndex + 1) % serverConfigurations.length;
    if (currentIndex === 0) {
    ...
    if (serverConfigurations[currentIndex].weight >= currentWeight) {
      return serverConfigurations[currentIndex];
    }
  }
}
